...
sealed class PaletteStrategy<T> {
    ...
    class Indirect<T>(
        ...
        operator fun get(entry: Int): T = palette[blocks[entry]]
...
class Palette<T>(
    var paletteStrategy: PaletteStrategy<T>,
) {
    ...
    operator fun set(
        ...
        when (val currentStrategy = paletteStrategy) {
            ...
            is PaletteStrategy.Indirect -> {
                val index =
                    if (currentStrategy.palette.indexOf(block) != -1) {
                        ...
                        if (currentStrategy.palette.size >= (1 shl currentStrategy.bitsPerEntry.toInt())) {
                            ...
                            if (currentStrategy.bitsPerEntry == 14.toUByte()) {
                                val blocks =
                                    MutableList(Chunk.CHUNK_WIDTH * Chunk.CHUNK_LENGTH * Chunk.CHUNK_SECTION_HEIGHT) { i ->
                                        currentStrategy[i]
                                    }
