...
fun <T : Nbt> namedTag(
    name: String,
    tag: T,
): NamedTag<T> = NamedTag(name, tag)

fun <T : Nbt> listTag(
    tagType: Nbt.TagType<T>,
    receiver: MutableList<T>.() -> Unit,
): Nbt.ListTag<T> {
...
class CompoundTagBuilder(
    val tags: MutableMap<String, Nbt> = mutableMapOf<String, Nbt>(),
) {
